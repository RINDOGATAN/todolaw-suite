...
export interface DSARCommunicationEmailInput {
  to: string;
...
export async function sendDSARCommunicationEmail(
  input: DSARCommunicationEmailInput
): Promise<void> {
  if (!resend) {
    logger.warn("RESEND_API_KEY not configured; skipping DSAR communication email", {
  ...
  const tEmail = await getTranslations({ locale, namespace: "dsarPublic.communicationEmail" });

  const statusUrl = `${brand.appUrl}/dsar/status/${input.publicId}`;
  const subjectLine = input.subject?.trim()
    ? tEmail("subjectWithCustom", { orgName: input.organizationName, subject: input.subject })
    : tEmail("subject", { orgName: input.organizationName });

  const bodyHtml = escapeHtml(input.content).replace(/\n/g, "<br />");
  ...
  try {
      ...
      html: `
        <div style="font-family: 'Helvetica Neue', Helvetica, Arial, sans-serif; max-width: 560px; margin: 0 auto; background: ${brand.colors.background}; border-radius: 12px; overflow: hidden;">
          ...
          <div style="padding: 32px 24px;">
            ...
            <p style="color: ${brand.colors.foreground}; font-size: 15px; line-height: 1.6; margin: 0 0 24px;">${escapeHtml(tEmail("intro", { orgName: input.organizationName }))}</p>
            <div style="background: ${brand.colors.muted}; border-radius: 8px; padding: 16px 20px; margin: 0 0 24px; color: ${brand.colors.foreground}; font-size: 14px; line-height: 1.6;">
              ${bodyHtml}
            </div>
            ...
            <p style="color: ${brand.colors.mutedForeground}; font-size: 13px; line-height: 1.6; margin: 28px 0 0;">${escapeHtml(tEmail("footer"))}</p>
          </div>
    ...
    logger.error("Failed to send DSAR communication email", error, {
      publicId: input.publicId,
    });
  }
}
